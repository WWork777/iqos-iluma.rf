...
export async function generateMetadata({ params }) {
  ...
  try {
    ...
    return {
      ...
      alternates: {
        canonical: `https://айкос-илюма.рф/products/product-info/${type}/${ref}`,
      },
      openGraph: {
        title: `Купить ${items.name} с доставкой по России`,
        description: items.description || `Купить ${items.name}`,
        url: `https://айкос-илюма.рф/products/product-info/${type}/${ref}`,
        images: [
          {
            url: items.image
              ? `https://айкос-илюма.рф/images/${items.image}`
              : `https://айкос-илюма.рф/favicon/web-app-manifest-512x512.png`,
            alt: items.name,
          },
        ],
      },
